...
par(mfrow=c(1,2))
lapply(seuratObjects, function(x) {hist(x$nFeature_RNA, 50, xlim= c(0,6000), main = unique(x$pool) , xlab = "nFeature RNA")
...
par(mfrow=c(1,2))
lapply(seuratObjects, function(x) {hist(x$percent_mt, 100, xlim= c(0,25),main = unique(x$pool), xlab = "% mitochondrial reads")
  abline(v = pctMt)})
...
write.csv(permDensityResults, 
          file.path(tableDir, "GenotypeLabelPermutationResults.csv"),
          row.names = F,
          quote = F)
...
permDensityAbs$quantile <- rowSums(permDensityAbs[,4:ncol(permDensityAbs)] < permDensityAbs$z)/(nPerm)

permDensityAbs$pval <- 1-permDensityAbs$quantile
permDensityAbs$pval_signed <- permDensityAbs$pval*sign(permDensityAbs$z)
...
png(file.path(plotDir, "UMAP_DifferentialDensityP_0_01.png"),
 ...
 pdf(file.path(plotDir, "UMAP_DifferentialDensityP_0_001.pdf"),
    height = 6, 
    width = 8)
